...
class DT_record():
    ...
    def __initcomponents_table(self):
        ...
        for i in range (len(fld_lst)):
            compdata[fld_lst[i].getOffset()]=component_data(fld_lst[i].getDataType().getLength())
    ...
    def update_stats(self, offset, cache_miss, version):
        ...
        if self.__dt.getComponentAt(offset).getDataType().getDisplayName() =="undefined":
            raise err_in_Toaddress("preventing updating stats for UNDEFINED fields")
    ...
    def setcomments(self, cb_option):
        ...
        for k in self.__components_table.keys():
            if cb_option=="wcb":
                self.__dt.getComponentAt(k).setComment(str(self.__components_table[k].getMisses_wcb()))
            elif cb_option=="wocb":
                self.__dt.getComponentAt(k).setComment(str(self.__components_table[k].getMisses_wocb()))
        return
    ...
    def set_savedmisses(self):
        ...
        for j in self.__components_table.keys():
            lst.append((j, self.__components_table[j].getMisses_wocb(), self.__components_table[j].getsize()))
...
class ghidra_bcpi():
    ...
    def update_stats(self, cache_miss, address):
        ...
        addr_reflst=lst_map[address]#addr_reflst is a list of tuple (dt, offset) for a given address
        cb_reflst=self.__getRefsInCB(address) #cb_reflst is a list of tuple (dt, offset) in cb of a given address
    ...
    def __update_from_refs(self, cache_miss, refs, version):
        ...
        for i in lst:#i is a single tuple
            try:
                cacheline=self.__main_update_phase(i, cache_miss, version, refs)
                if version=="addr":
    ...
    def print_summary(self):
        ...
        for i in self.__struct_mapp.keys():
            ...
            dt_lst.append((i.getName(), self.__struct_mapp[i].get_savedmisses()))
...
def unpickle_table(cp):
    ...
    pickleFile = open("/net/charm/usr/home/zahra/mantou/"+cp.getName()+".pkl", 'rb')
...
def main(currentProgram):
    ...
    data=read_csv("/net/charm/usr/home/zahra/zahra_rem/bcpinew/address_info.csv")
    gbcpi=ghidra_bcpi(currentProgram)
    unpickle_table(currentProgram)

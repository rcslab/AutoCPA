...
def AddReftoTable(ref):
	...
	lst_map[str(ref.getAddress())].append((dtPath.getPath() ,glb_offset, dtCat.getPath(), dtCat.getName()))
...
class ghidra_bcpi():
		...
		for dt in dataTypes:
			...
			for cmp in components:
				...
				for finder in finders:
					finder.findReferences(currentProgram, dt, fieldName, jc, ConsoleTaskMonitor())
				glb_offset=None
			glb_datatype=None
    ...
    def __filter_dt(self, iterator, lst):
    	reject_filters={"/std/", "/stdlib.h/", "/stdio.h/", "/_UNCATEGORIZED_/"}
    	pass_filters={"DWARF/"}
